...
import React, { useState, useRef } from 'react';
import { Video, ChevronRight, X, Upload, Loader2 } from 'lucide-react';
import { useNavigate } from 'react-router-dom'; // Note: For actual implementation
...
const LocalVideoCard = () => {
  ...
  // Implements exponential backoff for polling (matching YouTube's 50 attempts)
  const pollWithBackoff = async (taskId, maxAttempts = 50, initialInterval = 1000) => {
    ...
    while (!result && attempts < maxAttempts) {
      try {
        ...
        // Update progress if available (matching YouTube component behavior)
        ...
        if (statusData.status === 'completed') {
          ...
          if (!statusData.video_info || !statusData.transcript_info) {
            console.error('Response marked as completed but missing required data', statusData);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 2. Poll for completion with exponential backoff (matching YouTube)
